...
fn evaluate_expression_part2_2(expression: Expression) -> u64 {
    ...
    let add_op = expression.ops.iter().position(|o| *o == Op::Add);
    match add_op {
        Some(i) => {
...
fn evaluate_expression_part2(expression: Expression) -> u64 {
    ...
    let sum_ops: Vec<usize> = expression
        .ops
        .iter()
        .enumerate()
        ...
        .map(|(i, _)| i)
        .collect();
    let product_ops: Vec<usize> = expression
        .ops
        .iter()
        .enumerate()
    ...
    let sum_merged: Vec<(usize, usize, u64)> =
        sums.iter().fold(vec![], |mut xs, (i, left, right)| {
    ...
    for i in product_ops {
        ...
        let right = find_sum_expr(&sum_merged, i + 1).unwrap_or_else(|| {
            (
                i + 1,
                i + 1,
                evaluate_value(expression.values[i + 1].clone()),
    ...
    products
        ...
        .fold(
            vec![],
            |mut xs: Vec<(usize, usize, u64)>, (lower, upper, left, right)| {
...
fn parse_expression<I>(iterator: &mut I) -> Expression
where
...
fn parse_value<I>(iterator: &mut I) -> Value
where
    I: Iterator<Item = char>,
{
