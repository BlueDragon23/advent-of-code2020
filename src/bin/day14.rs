        ...
        |mut state, instruction| {
            match instruction {
                ...
                Instruction::Assignment(destination, value) => {
                    ...
                    let ps = iter
                        .iter()
                        .fold(vec![temp.clone()], |new: Vec<Vec<char>>, (i, _)| {
                            // copy state
                            let mut updated_0s = new
                                .iter()
                                .map(|xs| {
                    ...
                    let new_destinations = ps
                        .iter()
                        .map(|xs| {
                            u64::from_str_radix(xs.iter().collect::<String>().as_str(), 2).unwrap()
                        })
                        .collect::<Vec<_>>();
    ...
    let result = parsed.iter().fold(
        ...
        |mut state, instruction| match instruction {
            ...
            Instruction::Assignment(destination, value) => {
                ...
                let new_value = u64::from_str_radix(
                    bin.chars()
                        .zip(state.mask.iter())
                        .map(|(c, mask)| match mask {
                            Some(true) => '1',
                ...
                .unwrap_or_else(|_| panic!("Failed on binary {}", bin));
