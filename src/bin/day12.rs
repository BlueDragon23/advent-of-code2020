    ...
    let end_state = reader
        ...
        .fold(
            ...
            |state, instruction| match instruction {
                Instruction::North(d) => State {
                    waypoint_y: state.waypoint_y + d,
                    ..state
                },
                Instruction::South(d) => State {
                    waypoint_y: state.waypoint_y - d,
                    ..state
                },
                Instruction::East(d) => State {
                    waypoint_x: state.waypoint_x + d,
                    ..state
                },
                Instruction::West(d) => State {
                    waypoint_x: state.waypoint_x - d,
                    ..state
                },
                Instruction::Forward(d) => State {
                    x: state.x + state.waypoint_x * d,
                    y: state.y + state.waypoint_y * d,
                    ..state
                },
                Instruction::Left(d) => match d {
                    0 => State {
                        waypoint_x: state.waypoint_x,
                        waypoint_y: state.waypoint_y,
                        ..state
                    },
                    90 => State {
...
fn update_part1(state: State, instruction: Instruction) -> State {
    match instruction {
        ...
        Instruction::Left(d) => State {
            direction: (state.direction + d) % 360,
            ..state
        },
        Instruction::Right(d) => State {
            direction: (state.direction - d + 360) % 360,
            ..state
        },
    }
}
