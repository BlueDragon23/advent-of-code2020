...
fn main() {
    ...
    let parsed = reader.lines().map(|line| line.unwrap()).fold(
        State {
    ...
    names.sort_by(|&a, &b| {
        field_columns[a]
            .len()
            .partial_cmp(&field_columns[b].len())
            .unwrap()
    });
...
fn parse_field(line: &str) -> Field {
    ...
    Field {
        ...
        lower_range: (
            range_captures[1].parse().unwrap(),
            range_captures[2].parse().unwrap(),
        ),
        upper_range: (
            range_captures[3].parse().unwrap(),
            range_captures[4].parse().unwrap(),
        ),
    }
}
...
fn parse_ticket(line: &str) -> Ticket {
    Ticket {
        values: line.split(",").map(|n| n.parse().unwrap()).collect(),
    }
}
