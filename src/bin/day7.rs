...
fn main() {
    ...
    let (graph, _) = reader.lines().map(|line| line.unwrap()).fold(
        (DiGraph::new(), HashMap::new()),
        |(mut graph, mut map): (_, HashMap<Node, NodeIndex>), line| {
            let mut parts = line.split(" contain ");
            let from = Node::parse(parts.next().unwrap()).unwrap();
            let from_index = *map
                .entry(from.clone())
                .or_insert(graph.add_node(from.clone()));
    ...
    let index = graph
        .node_indices()
        .find(|i| graph[*i].colour == "shiny gold")
        .unwrap();
    ...
    while let Some((edge_index, node_index)) = walker.next(&graph) {
        ...
        if children == 0 {
            ...
            println!(
                ...
                weight,
                graph.node_weight(node_index)
            );
            sum += weight
        } else {
            println!(
                "{:?} is parent of {} {:?} bags with {} children",
                graph.node_weight(node),
                weight,
                graph.node_weight(node_index),
                children
