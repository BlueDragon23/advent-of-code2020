...
fn main() {
    ...
    let messages = lines.iter().skip(rules.len() + 1).collect::<Vec<_>>();
...
fn valid_simplified(forty_two: &String, thirty_one: &String, line: &str) -> bool {
    ...
    let regex = Regex::new(
        format!(
            "^(?P<left>{}{}+)(?P<right>{}+)$",
            forty_two, forty_two, thirty_one
        )
        .as_str(),
    )
    .unwrap();
...
fn simplify_rule(rules: &HashMap<usize, RuleType>, rule_id: usize) -> HashSet<String> {
    ...
    match rule {
        ...
        RuleType::Sequence(sequence) => simplify_sequence(rules, sequence),
        RuleType::Alternation(first, second) => vec![
            simplify_sequence(rules, first),
            simplify_sequence(rules, second),
        ]
        .into_iter()
        .concat(),
    }
...
fn is_valid(
    ...
    match rules[&rule_id] {
        ...
        RuleType::Alternation(left, right) => {
            log::debug!(
                "Checking index {} against left alternation {:?}",
                index,
                left
            );
            // If we're on an alternation, check the left side then the right
            let (mut result, mut new_index) =
                is_valid_sequence(message, rules, index, left.first, left.second, left.third);
            if !result {
                log::debug!(
                    "Checking index {} against right alternation {:?}",
                    index,
                    right
                );
                let (new_result, right_index) = is_valid_sequence(
                    message,
                    rules,
                    index,
                    right.first,
...
fn parse_rule(line: &str) -> (usize, RuleType) {
    ...
    let sequence_pattern =
        Regex::new(r"(?P<first>\d+)( (?P<second>\d+)( (?P<third>\d+))?)?").unwrap();
    let (id_str, contents) = line.split_once(": ").unwrap();
    let id = id_str.parse::<usize>().unwrap();
    if let Some(terminal) = terminal_pattern.captures(contents) {
        return (id, RuleType::Terminal(terminal[1].chars().next().unwrap()));
    }
